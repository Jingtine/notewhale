...
说明：当前中期 Demo 前端部署在 Vercel，后端部署在 Render，主要用于功能展示。由于部署平台位于海外，国内网络环境下可能存在访问不稳定的情况。建议使用VPN访问网站体验完整功能。目前已接入 DeepSeek 文本模型，并探索 GLM 视觉模型在 DDL 图片识别中的应用；由于模型额度、云端部署和文件解析稳定性等因素，AI 相关功能仍处于持续调试优化阶段。后续计划将项目迁移至国内云服务，完成域名解析、备案和稳定部署，或打包成可本地运行的程序版本。
...
或更推荐访问目前的海外部署网站体验完整功能。

如果需要体验 AI 笔记生成，可在后端目录下创建 `.env` 文件，或直接在系统环境变量中配置以下内容：

```text
NOTEWHALE_TEXT_API_URL=https://api.deepseek.com/chat/completions
NOTEWHALE_TEXT_API_KEY=你的 DeepSeek API Key
NOTEWHALE_TEXT_MODEL=deepseek-chat
```

如果需要体验 DDL 图片识别等视觉模型功能，可根据实际使用的视觉模型配置：
...
如果未配置 AI Key，AI 笔记和视觉识别相关功能可能无法正常生成，但不影响课程、资料、笔记和 DDL 等基础功能体验。
...
如果使用 macOS / Linux，可将激活虚拟环境命令替换为：
...
1. 进入线上系统首页。
2. 注册或登录账号。（已建 体验账号1：666666.com  key：666666 ，为演示使用账号，可凭此登录访问）
3. 在首页新建课程。
4. 使用文件夹对课程进行分类。
5. 点击课程卡片进入课程详情页。
6. 在课程页面上传课程资料。
...
This README was prepared by Jingtine Lee.

This is an interim trial version. If you have any questions, please contact: 251250208@smail.nju.edu.cn